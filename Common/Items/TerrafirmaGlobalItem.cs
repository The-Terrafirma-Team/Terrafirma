            ...
            if (item.rare == ModContent.RarityType<FinalQuestRarity>())
            {
                ...
                if (line.Name == "ItemName")
                {
                    if (Main.timeForVisualEffects % 15 == 0)
                    {
                        ...
                        ParticleSystem.AddParticle(new BigSparkle(), new Vector2(Main.rand.NextFloat(line.Text.Length * 9.5f), Main.rand.NextFloat(20f)) - Main.screenPosition,null,line.Color,ParticleLayer.UI);
                    }
                }
