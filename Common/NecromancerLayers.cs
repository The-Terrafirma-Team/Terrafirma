    ...
    public class NecromancerLayerBack : NecromancerLayer
    {
        protected override void Draw(ref PlayerDrawSet drawInfo)
        {
            ...
            for (int i = 0; i < (scythe.DamageDealt / scythe.DamagePerSoul); i++)
            {
                ...
                float halfCircle = (float)Math.Sin((timer * 0.03f) + 1.521229);
                if (halfCircle > 0)
                    drawInfo.DrawDataCache.Add(new DrawData(tex.Value, position + new Vector2(circle * 28f, Math.Abs(halfCircle) * 2f), new Rectangle(tex.Height() * (i % 2 == 0 ? 0 : 1), 0, tex.Height(), tex.Height()), Color.Lerp(Color.White,Color.Gray,halfCircle) * 0.8f, 0, new Vector2(tex.Width() / 4, tex.Height() / 2), 0.65f - halfCircle * 0.15f, SpriteEffects.None));
            }
        }
