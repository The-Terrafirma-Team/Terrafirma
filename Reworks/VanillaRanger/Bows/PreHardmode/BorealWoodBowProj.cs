...
{
    public class BorealWoodBowProj : DrawnBowTemplate
    {
        ...
        public override void Shoot(IEntitySource source, Vector2 position, Vector2 velocity, int type, int damage, float knockback, float power)
        {
            Projectile p = Projectile.NewProjectileDirect(
                source, 
                position,
                Vector2.Lerp(velocity, velocity * 1.5f, (float)Math.Pow(power, 3)), 
                type, 
                (int)MathHelper.Lerp(damage, damage * 2, (float)Math.Pow(power,3)), 
                knockback, 
                player.whoAmI);
            p.GetGlobalProjectile<VanillaBowProjectileEdits>().sourceWeapon = ItemID.BorealWoodBow;
            p.GetGlobalProjectile<VanillaBowProjectileEdits>().power = power;
            p.penetrate++;
        } 
    }
}
