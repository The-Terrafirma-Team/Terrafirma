...
{
    internal class DiamondCane : ModItem
    {
        ...
        {
            CreateRecipe().AddIngredient(ItemID.PlatinumBar, 10).AddIngredient(ItemID.Diamond, 8).AddTile(TileID.Anvils).Register();
        }
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            if (player == Main.LocalPlayer)
            {
                ...
                Projectile.NewProjectile(source, new Vector2(WorldX, WorldY - PushUpY - 2), Vector2.Zero, type, damage, 0, player.whoAmI, 0, 0, 5);
